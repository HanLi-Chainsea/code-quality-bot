...
**北極星（這套系統的終極判準）**
> **一位資深工程師可以完全信任：凡是他會提出的風險，這套也會提出。**
> （「我」＝資深工程師，「他」＝這套 review 系統。）

信任是**雙向**的，缺一不可：
- **不漏（recall vs 資深）**：資深會抓的，它也要抓到 —— 否則它不是安全網，資深仍得自己全看一遍。
- **不吵（precision vs 資深）**：它提的，資深都認同值得提 —— 否則雜訊侵蝕信任，資深開始略過它的留言。

一旦資深發現「它漏了我會抓的」**或**「它一堆我根本不在意的」，信任就破。兩者同等致命。

**衍生的具體目標**
- **深度**：review 要有跨檔、影響範圍（blast radius）的認知 —— 這是「不漏」資深會抓的跨檔風險的前提。
- **訊號**：只報值得提的（朝 **FP 率 < 10%**，見 §4：>10% 工程師就嫌吵、>50% 直接無視）—— 這是「不吵」。
...
- **Go/No-Go（對齊 §8 北極星「可完全信任」）**：資深盲評輸出後願意說「**我會抓的它沒漏、它提的我都認同**」。具體至少要 (a) blast-radius 版抓到 diff-only 漏掉的跨檔/影響風險；(b) 對資深清單的 `blocker`/`major` **recall 逼近滿分（不漏）**；(c) **FP（含 diff-myopia）< 10%（不吵）**。人眼判。
- 過了才往下；沒過就便宜地學到「這條路不夠」（並從漏/誤殺的案例回推是 context 還是 verify 的問題）。
...
## 8. 成功指標（對齊北極星：與資深工程師的判斷一致）

核心 eval：挑 N 個真實 MR，由**資深工程師（你）先獨立寫下「我會提出的風險」**，再跟這套的輸出比對兩個方向：

- **不漏（recall vs 資深）**：資深列的風險，這套抓到幾成 —— `blocker`/`major` 要**逼近 100%**，否則當不了安全網。漏掉的逐一記下來（是 context 不夠？還是被 verify 誤殺？）。
- **不吵（precision vs 資深）**：這套提的，資深認同「值得提」的比例 —— 目標 **FP < 10%**，diff-myopia 那類（§4）單獨計。
- **深度佐證（A/B）**：這套抓到、而 diff-only baseline 漏掉的跨檔/影響型風險 —— 證明 blast-radius 的價值。

**達標定義**：樣本 MR 上，資深看完輸出能說「**這些我都認同，而且我會抓的它沒漏**」= 達到「可完全信任」。Phase 1 的 Go/No-Go 即以此為準。
